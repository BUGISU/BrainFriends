...
function ResultContent() {
  ...
  const [serverHistory, setServerHistory] = useState<TrainingHistoryEntry[] | null>(null);
  ...
  }, [currentTrainingMode, historyRefreshKey, historySourceRows, patientForHistory]);
  ...
  useEffect(() => {
    ...
    const loadServerHistory = async () => {
      try {
        ...
        console.error("[result-self] failed to load server history", error);
        if (!cancelled) {
