...
function Step6Content() {
  ...
  const checkAnswer = () => {
    ...
    const persistStep6Entry = (entry: Record<string, unknown>) => {
      ...
      byIndex.set(currentIndex, entry);
    ...
    if (isFinalCorrect) {
      ...
      if (nextFailedAttempts >= 2) {
        ...
        setPraiseMessage("두 번 이상 어려워 다음 문제로 넘어갑니다.");
        setPhase("review");
        return;
      }
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      try {
        ...
        const completedCount = scoreRows.filter((row) => row?.text).length;
