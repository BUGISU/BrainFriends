            ...
            <section className="rounded-[30px] border border-emerald-100 bg-[#fbfefc] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-8">
              ...
              <div className="mt-6 space-y-5">
                <SymmetryRow
                  label="좌측 구강 근육 활성도"
                  before={67}
                  after={67 + mouthImprovement}
                  feedback={`${mouthImprovement}% 증가`}
                />
                <SymmetryRow
                  label="좌측 안륜근 반응도"
                  before={72}
                  after={72 + eyeImprovement}
                  feedback={`${eyeImprovement}% 증가`}
                />
                <SymmetryRow
                  label="표정-발성 협응 지수"
                  before={78}
                  after={Math.min(98, Math.round(facialSymmetryScore))}
                  feedback={`${Math.max(8, Math.round(facialSymmetryScore - 78))}% 향상`}
                />
              </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[0.95fr_1.05fr]">
            <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-8">
              ...
              <p className="mt-4 text-base font-medium text-slate-500">
                {maskedAge} · 같은 연령대 안면 마비 진단 참여자와 비교한 위치입니다.
              </p>

              <div className="mt-5 space-y-3">
                {result.rankings.map((row, idx) => (
                  <div
                    key={`${row.name}-${idx}`}
              ...
              <div className="mt-6 space-y-4 text-lg leading-relaxed text-slate-700">
                ...
                <div className="rounded-[24px] border border-emerald-200 bg-white/70 p-4">
                  ...
                  <p className="mt-2 text-base font-medium text-slate-700">
                    오늘처럼 노래 리듬에 맞춰 입 모양을 크게 열어 주시면, 안면 좌우 협응 회복에 도움이 됩니다.
                    같은 흐름으로 꾸준히 진행하면 더 안정적인 표정 반응을 기대할 수 있습니다.
                  </p>
                </div>
              </div>
            </section>
